...
<html>
    ...
    <body>
        <div class="position-absolute top-50 start-50 translate-middle" style="width: 75%;">
            ...
            <form action="{{ url('/dashboard/'.$mahasiswa->nrp) }}" method="POST" style="margin: 3rem;">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control" type="text" name="nama" value="{{ $mahasiswa->nama }}" placeholder="Masukkan Nama">
                    @error('nama') <span class="alert alert-danger">{{ $message }}</span> @enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Email</label>
                    <input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="email" value="{{ $mahasiswa->email }}"placeholder="name@ecxample.com">
